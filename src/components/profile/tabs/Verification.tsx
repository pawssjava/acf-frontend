...
export default function Verification({ user, onUpdate }: Props) {
  ...
  const handleFile = async (file: File) => {
    ...
    try {
      ...
      setError(msg || 'Ошибка при загрузке документа');
    } finally {
      setUploading(false);
    ...
    <div className={styles.wrap}>
      {user.isVerified ? (
        <div className={styles.bannerVerified}>
          ...
          <div>
            <div className={styles.bannerTitle}>Профиль подтвержден</div>
            <div className={styles.bannerSub}>Теперь вы можете регистрироваться и участвовать в турнирах</div>
          </div>
        </div>
      ) : (
        <div className={styles.bannerUnverified}>
          ...
          <div className={styles.bannerText}>
            <div className={styles.bannerTitle}>Профиль не подтвержден</div>
            <div className={styles.bannerSub}>Подтвердите профиль чтобы участвовать в турнирах</div>
          </div>
      ...
      >
        ...
        <p className={styles.hint}>нажмите, чтобы выбрать документ</p>
        <p className={styles.dropText}>Перетащите PDF-файл сюда</p>
        ...
        <p className={styles.fine}>
          Принимаются только PDF-файлы. Подойдет официальный документ, подтверждающий личность, например, цифровой документ из eGov, Казрі или другого официального сервиса
        </p>
